...
Void task1(UArg arg0, UArg arg1)
{
    ...
    while (1) {
        if (!MAP_GPIO_getInputPinValue(GPIO_PORT_P1, GPIO_PIN1)) {
            ...
            MAP_GPIO_setOutputHighOnPin(GPIO_PORT_P1, GPIO_PIN0); // Turn on the red LED
        } else {
            // Button S1 is released
            MAP_GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN0);  // Turn off the red LED
        }
    }
}
...
Void task2(UArg arg0, UArg arg1)
{
    ...
    while (1) {
        if (!MAP_GPIO_getInputPinValue(GPIO_PORT_P1, GPIO_PIN4)) {
            ...
            MAP_GPIO_setOutputHighOnPin(GPIO_PORT_P2, GPIO_PIN1); // Turn on the green LED
        } else {
            // Button S2 is released
            MAP_GPIO_setOutputLowOnPin(GPIO_PORT_P2, GPIO_PIN1);  // Turn off the green LED
        }
    }
}
